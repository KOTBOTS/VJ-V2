...
imdb = IMDb() 
myclient = pymongo.MongoClient(Config.DATABASE)
mydb = myclient[Config.BOT_NAME]
...
def insert(chat_id):
            ...
            try:
            	aambro.insert_one(user_det)
            except:
            	pass

def set(chat_id,lg_code):
	 aambro.update_one({"_id":chat_id},{"$set":{"lg_code":lg_code}})
...
async def google_search(bot, update):
    ...
    text=Config.SPELLING_MODE.format(update.from_user.mention),
    parse_mode="html",
    reply_markup=Import.Markup([[Import.Button("🔍 Search Google 🔎", url="https://www.imdb.com/search/")]]),
...
class Database:
    def __init__(self):
        ...
        self.db = self._client[Config.BOT_NAME]
    ...
    Donlee_bt = InlineKeyboardMarkup( [[ InlineKeyboardButton("💞SOURCE CODE💞", url="https://t.me/KOT_SOURCE_CODE")]])
